...
export class QuoteFileRepository {
  ...
  async createQuoteFile(data: Prisma.QuoteFileUncheckedCreateInput): Promise<QuoteFile> {
    return this.prisma.quoteFile.create({ data });
  }

  async findQuoteFiles(tenantId: string, quoteId: string): Promise<QuoteFile[]> {
    return this.prisma.quoteFile.findMany({
      where: { tenantId, quoteId, deletedAt: null },
    });
  }

  async findQuoteFileById(tenantId: string, id: string): Promise<QuoteFile | null> {
