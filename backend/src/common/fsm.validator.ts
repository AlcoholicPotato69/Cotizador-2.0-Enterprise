...
export class FsmValidator {
  validateTransition(
    entityName: 'Quote' | 'Contract' | 'Invoice' | 'Payment',
    currentState: string,
    targetState: string,
  ) {
    ...
    if (!allowedTransitions) {
      throw new BadRequestException(
        `Entity ${entityName} has unknown current state: ${currentState}`,
      );
    }
  ...
  validateSignatureEligibility(currentState: string) {
    if (
      ...
      ).includes(currentState as ContractStatus)
    ) {
      throw new BadRequestException(
        `Cannot add signature to contract in state: ${currentState}`,
      );
    }
  }
  ...
  validateInvoiceGenerationEligibility(contractState: string) {
    if (contractState !== ContractStatus.ACTIVE) {
      throw new BadRequestException(
        `Invoice can only be generated for ACTIVE contracts. Current status: ${contractState}`,
      );
    }
  }
}
