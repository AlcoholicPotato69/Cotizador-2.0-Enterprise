...
describe('PaymentsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: PaymentEvidencesRepository,
    ...
    evidencesRepo = module.get(PaymentEvidencesRepository);
  ...
  describe('submitPayment', () => {
    it('should create payment, save evidence via repo and transition to UNDER_REVIEW', async () => {
      const dto = {
        ...
        evidenceUrl: 'http://evidence.url/1',
        currencyCode: 'USD',
      };
      ...
      await tenantContext.run({ tenantId: 'tenant-1' }, async () => {
        ...
        expect(evidencesRepo.create).toHaveBeenCalledWith(
          expect.anything(),
          expect.objectContaining({
            paymentId: 'pay-1',
            url: dto.evidenceUrl,
  ...
  describe('approvePayment', () => {
    it('should approve payment', async () => {
      paymentsRepo.findByIdForUpdate.mockResolvedValue({
        id: 'pay-1',
        invoiceId: 'inv-1',
    ...
    it('should throw exception if FSM blocks approval on invalid payment status', async () => {
      paymentsRepo.findByIdForUpdate.mockResolvedValue({
        id: 'pay-1',
        invoiceId: 'inv-1',
        paymentAmount: new Prisma.Decimal(50),
        status: PaymentStatus.APPROVED,
      } as any);
      ...
      await tenantContext.run({ tenantId: 'tenant-1' }, async () => {
        await expect(service.approvePayment('pay-1')).rejects.toThrow(
