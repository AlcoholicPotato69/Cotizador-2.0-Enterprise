  ...
  async evaluateEligibility(
    ...
    if (!tenantId) {
      throw new ForbiddenException(
        'Tenant context required for eligibility check',
      );
    }
    ...
    if (!client) {
      throw new NotFoundException(
        `Client ${clientId} not found in current tenant`,
      );
    }
    ...
    if (
      ...
      throw new ForbiddenException(
        `Client ${clientId} is in a non-eligible status: ${client.status}`,
      );
    }
    ...
    switch (transactionType) {
      case TransactionType.CONTRACT_GENERATION:
        if (client.isContractBlocked) {
          throw new ForbiddenException(
            `Client ${clientId} is blocked from generating new contracts`,
          );
        }
        break;
      case TransactionType.INVOICE_GENERATION:
        if (client.isInvoiceBlocked) {
          throw new ForbiddenException(
            `Client ${clientId} is blocked from generating new invoices`,
          );
        }
        break;
      case TransactionType.PAYMENT_PROCESSING:
        if (client.isPaymentBlocked) {
          throw new ForbiddenException(
            `Client ${clientId} is blocked from processing payments`,
          );
        }
        break;
      case TransactionType.QUOTE_CREATION:
        ...
        if (!client.isTaxValidated) {
          throw new ForbiddenException(
            `Client ${clientId} is not tax validated. Cannot create quotes.`,
          );
        }
        break;
    }
