...
export class AvailabilityEngineService {
  ...
  async reserveSpace(
    ...
    if (!ctx || !ctx.tenantId)
      throw new ConflictException('Tenant context is missing');
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      const space = await this.spacesRepo.findByIdForUpdate(
        tx,
        ctx.tenantId,
        request.spaceId,
      );
      ...
      const blockedDays = Array.isArray(space.diasBloqueados)
        ? space.diasBloqueados
        : [];
