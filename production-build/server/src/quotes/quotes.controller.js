...
let QuotesController = class QuotesController {
    ...
    async createQuote(req, body) {
        return tenant_context_1.tenantContext.run({ tenantId: req.user.tenantId, userId: req.user.id, role: req.user.role }, () => this.service.create(body));
    }
    async updateStatus(req, id, status) {
        return tenant_context_1.tenantContext.run({ tenantId: req.user.tenantId, userId: req.user.id, role: req.user.role }, () => this.service.updateStatus(id, status));
    }
};
