<template>
  <main class="dark min-h-[100dvh] w-full bg-surface-900 relative overflow-hidden flex items-center justify-center p-4 md:p-12">
    ...
    <div class="relative z-10 w-full max-w-7xl grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
      ...
      <div class="p-2 rounded-[2.5rem] border border-white/5 bg-surface-800/40 backdrop-blur-2xl" style="box-shadow: var(--shadow-ambient)">
        <div class="bg-surface-900/80 rounded-[calc(2.5rem-0.5rem)] p-8 md:p-12 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] flex flex-col space-y-6">
          <div class="space-y-2">
          ...
          <form @submit.prevent="handleLogin" class="space-y-5">
            <input v-model="email" type="email" required placeholder="Correo corporativo" class="w-full bg-surface-800 border border-white/10 rounded-2xl px-6 py-4 text-white focus:ring-1 focus:ring-primary-500 transition-all outline-none" />
            <input v-model="password" type="password" required placeholder="Contraseña" class="w-full bg-surface-800 border border-white/10 rounded-2xl px-6 py-4 text-white focus:ring-1 focus:ring-primary-500 transition-all outline-none" />
            ...
            <button type="submit" :disabled="isLoading" class="group w-full flex items-center justify-between bg-primary-500 hover:brightness-110 text-white rounded-full pl-8 pr-2 py-2 mt-4 transition-all duration-700 active:scale-[0.98] disabled:opacity-50">
              ...
              <div class="w-10 h-10 rounded-full bg-surface-0/20 flex items-center justify-center transition-transform group-hover:translate-x-1 group-hover:-translate-y-[1px]">↗</div>
            </button>
          </form>
        </div>
      </div>
